...
// 5. route
commands.route = function (args) {
  const taskDir = resolveTaskDir(args);
  const config = readJSON(path.join(taskDir, 'config.json'));
  const stage = config.stage;
  const phases = config.phases || {};

  // Count gray areas from CONTEXT.md
  let grayAreas = 0;
  try {
    const ctx = fs.readFileSync(path.join(taskDir, 'CONTEXT.md'), 'utf8');
    const graySection = ctx.split('Gray Areas Remaining')[1];
    if (graySection) {
      const unchecked = (graySection.match(/^- \[ \]/gm) || []).length;
      grayAreas = unchecked;
    }
  } catch (e) {}
  ...
  if (stage === 'discussion') {
    if (grayAreas > 0) {
      return out({ next_step: 'discuss', pipeline: 'main', resume: false });
    }
    return out({ next_step: 'research', pipeline: 'main', resume: false });
  }
  ...
  if (stage === 'research') {
    ...
    return out({ next_step: 'plan', pipeline: 'main', resume: false });
  }
  ...
  if (stage === 'execution' || stage === 'planning') {
    ...
    if (status === 'pending') {
      const planPath = path.join(taskDir, 'phases', `phase-${padPhase(current)}`, 'PLAN.md');
      // Also check for decimal fix phases
      let hasFixPhases = false;
      try {
        const entries = fs.readdirSync(path.join(taskDir, 'phases'));
        hasFixPhases = entries.some(e => e.match(new RegExp(`^phase-${padPhase(current)}\\.\\d+$`)));
      } catch (e) {}

      if (hasFixPhases) {
        // Find incomplete fix phases
        return out({ next_step: 'execute', pipeline: 'phase-execution', resume: false });
      }
      if (fs.existsSync(planPath)) {
        return out({ next_step: 'execute', pipeline: 'phase-execution', resume: false });
      }
      return out({ next_step: 'plan', pipeline: 'phase-execution', resume: false });
    }

    if (status === 'executing') {
      return out({ next_step: 'execute', pipeline: 'phase-execution', resume: true });
    }

    if (status === 'executed') {
      return out({ next_step: 'review', pipeline: 'phase-execution', resume: false });
    }

    if (status === 'completed') {
      // Check for decimal fix phases first
      try {
        const entries = fs.readdirSync(path.join(taskDir, 'phases'));
        const fixPhases = entries
          .filter(e => e.match(new RegExp(`^phase-${padPhase(current)}\\.\\d+$`)))
          .sort();
      ...
      if (current >= total) {
        return out({ next_step: 'complete', pipeline: 'main', task_complete: true });
      }
      // Advance to next phase
      return out({ next_step: 'plan', pipeline: 'phase-execution', resume: false, advance_phase: true });
    }
  }

  fail(`Unknown state: stage=${stage}, status=${phases.current_status}`);
};
