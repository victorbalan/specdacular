...
Use sequential execution with `subagent_type="specd-codebase-mapper"` and `run_in_background=true` for parallel execution.
...
Use ask_user_question for each project (or batch if 3 or fewer):
- header: "{project-name}"
...
Ask user for additional docs (same ask_user_question as check_existing_docs).
...
Use sequential execution with `subagent_type="specd-codebase-mapper"` and `run_in_background=true`.
