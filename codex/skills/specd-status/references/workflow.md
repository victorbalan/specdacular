...
Use `find` to check if `.specd/features/*/config.json` matches anything.
...
1. Use `find` to find `{project-path}/.specd/features/*/config.json`
...
Print the formatted dashboard directly. No file writes. No Task agents. No ask_user_question.
