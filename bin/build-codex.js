...
function replacePathRefs(content) {
  // Replace @~/.claude/specdacular/ references → See references/
  let result = content.replace(/@~\/\.claude\/specdacular\/[^\s<)]+/g, '(see references/)');
  // Replace @/Users/.../specdacular/ absolute path references
  result = result.replace(/@\/[^\s<)]*specdacular\/[^\s<)]+/g, '(see references/)');
  // Replace remaining @~/.claude/ references
  result = result.replace(/@~\/\.claude\/[^\s<)]+/g, '(removed for Codex)');
  // Replace backtick-wrapped ~/.claude/ paths
  result = result.replace(/`~\/\.claude\/([^`]+)`/g, '`.codex/$1`');
  // Replace bare ~/.claude/ paths (not in backticks, not @-prefixed)
